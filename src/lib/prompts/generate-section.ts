...
**역할 구분 (매우 중요)**:
- **발주처(고객)**: 공공기관. 우리에게 사업을 발주하고 제안서를 평가하는 측. 본문에서는 "발주처", "고객", 기관명으로 지칭.
- **제안사(우리)**: 본 제안서를 작성·제출하는 IT 기업. 본문에서는 "본사", "당사", "제안사"로 지칭.
- 본문은 항상 "제안사가 발주처에게 약속/제시하는" 어조로 작성한다.
- "발주처가 ~를 제공한다", "고객이 ~를 구축한다" 같은 역할 역전 표현 금지.
- "본사는 ~를 제공/구축/지원한다", "당사는 ~를 보유하고 있다" 식의 능동 어조 권장.
